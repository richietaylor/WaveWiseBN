...
#     days_per_request = ceil(total_days / 10)  # Divide into 10 chunks
#     print(f"Total days to fetch: {total_days}")
#     print(f"Days per API request: {days_per_request}")

#     # Prepare a list to hold all processed data
#     all_processed_data = []

#     # Split the date range into 10 chunks
#     for i in range(10):
...
#         print(f"\nAPI Request {i+1}/10: Fetching data from {chunk_start_date.format('YYYY-MM-DD')} to {chunk_end_date.format('YYYY-MM-DD')}")
...
#         # Optional: Sleep to respect rate limits (ensure not to exceed 10 requests per day)
#         time.sleep(1)  # Adjust as needed based on API rate limits

#     # Save all data to CSV
#     save_to_csv(all_processed_data, sorted(set(params.split(','))), output_file=output_file)
...
#     # Collect the historical weather data in 10 requests
